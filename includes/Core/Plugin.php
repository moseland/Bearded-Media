...
final class Plugin {
	...
	private function init_hooks(): void {
		...
		add_filter( 'media_row_actions', array( $this, 'inject_list_view_actions' ), 10, 2 );
	}
				...
				'show_in_rest'      => array(
					'schema' => array(
						...
						'properties' => array(
							...
							'openrouter_key' => array( 'type' => 'string' ),
							'openai_key'     => array( 'type' => 'string' ),
							'together_key'   => array( 'type' => 'string' ),
						),
					),
				),
			)
	...
	public function sanitize_settings( array $input ): array {
		...
		if ( isset( $input['manual_models'] ) && is_array( $input['manual_models'] ) ) {
			$manual_models = array();
			foreach ( $input['manual_models'] as $m ) {
				$manual_models[] = array(
					'provider'   => sanitize_text_field( $m['provider'] ?? '' ),
					'label'      => sanitize_text_field( $m['label'] ?? '' ),
					'value'      => sanitize_text_field( $m['value'] ?? '' ),
					'capability' => sanitize_text_field( $m['capability'] ?? '' ),
				);
			}
			$sanitized['manual_models'] = $manual_models;
		}
	...
	public function sanitize_keys( array $input ): array {
		return array(
			...
			'openrouter_key' => sanitize_text_field( (string) ( $input['openrouter_key'] ?? '' ) ),
			'openai_key'     => sanitize_text_field( (string) ( $input['openai_key'] ?? '' ) ),
			'together_key'   => sanitize_text_field( (string) ( $input['together_key'] ?? '' ) ),
		);
	}
	...
	/**
	 * Append an "AI Editor" shortcut action below file items in List View.
	 *
	 * @param array<string, string> $actions Existing row actions.
	 * @param \WP_Post              $post    The attachment post object.
	 * @return array<string, string> Modified row actions.
	 */
	public function inject_list_view_actions( array $actions, \WP_Post $post ): array {
		if ( wp_attachment_is_image( $post->ID ) ) {
			$actions['bearded_ai_edit'] = sprintf(
				'<a href="#" class="bearded-media-list-action" data-id="%d" data-url="%s">%s</a>',
				absint( $post->ID ),
				esc_url( wp_get_attachment_url( $post->ID ) ),
				esc_html__( 'AI Editor', 'bearded-media' )
			);
		}
		return $actions;
	}
