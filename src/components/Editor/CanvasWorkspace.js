	...
	(
		...
		const drawCanvas = useCallback(
			( showUI = true ) => {
				...
				if ( maskImageRef.current ) {
					// Draw persistent mask back onto mask canvas
					mCtx.drawImage( maskImageRef.current, 0, 0 );

					// Render a red transparent visual overlay on the main screen canvas
					...
					if ( tempCtx ) {
						...
						for ( let i = 0; i < data.length; i += 4 ) {
							// Check if pixel is white-ish (r > 100, g > 100, b > 100)
							if (
								data[ i ] > 100 &&
								data[ i + 1 ] > 100 &&
								data[ i + 2 ] > 100
								...
								data[ i + 1 ] = 0; // G
								data[ i + 2 ] = 0; // B
								data[ i + 3 ] = 128; // Alpha opacity (50%)
							} else {
								data[ i + 3 ] = 0; // Transparent for background black pixels
							}
						}
		...
		useEffect( () => {
			if ( maskData ) {
				...
				// Physically clear the mask canvas back to solid black
		...
		const handleMouseUp = () => {
			...
			if ( activeTool === 'brush' && context && maskContext ) {
				...
				if ( needsMask ) {
					...
					onBrushBake( canvasRef.current.toDataURL( 'image/jpeg' ) );
				}
