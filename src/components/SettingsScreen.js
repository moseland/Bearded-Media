...
const App = () => {
	...
	useEffect( () => {
		const fetchSettings = async () => {
			...
			try {
				...
				if (
					Array.isArray(
						wpSettings.bearded_media_settings?.manual_models
					)
				) {
					resolvedModels =
						wpSettings.bearded_media_settings.manual_models;
					...
					wpSettings.bearded_media_settings?.manual_models &&
					typeof wpSettings.bearded_media_settings.manual_models ===
						'object'
				) {
					resolvedModels = Object.values(
						wpSettings.bearded_media_settings.manual_models
					);
				}
				...
				if ( wpSettings ) {
					...
					setApiKeys( {
						...
						openrouter_key:
							wpSettings.bearded_media_api_keys?.openrouter_key ??
							'',
					} );
				}
			} catch ( err ) {
	...
	const handleAddModel = () => {
		if ( ! newModel.label || ! newModel.value ) {
			alert(
				__( 'Please provide both a label and a slug.', 'bearded-media' )
			);
			return;
		}
		...
		setSettings( ( prev ) => ( {
			...prev,
			manual_models: [ ...prev.manual_models, newModel ],
		} ) );
	...
	const handleDeleteModel = ( index ) => {
		setSettings( ( prev ) => {
			const updated = [ ...prev.manual_models ];
			updated.splice( index, 1 );
			return { ...prev, manual_models: updated };
		} );
	};
				...
				{ activeTab === 'models' && (
					<>
						<SectionCard
							...
							icon={ <BoltIcon /> }
							description={ __(
								'Manually add an AI model to make it available in your generation tools.',
								'bearded-media'
							) }
						>
							<div
							...
							>
								<div className="input-field">
									<label htmlFor="new_model_provider">
										{ __( 'Provider', 'bearded-media' ) }
									</label>
									<select
										id="new_model_provider"
								...
								<div className="input-field">
									<label htmlFor="new_model_capability">
										{ __( 'Capability', 'bearded-media' ) }
									</label>
									<select
										id="new_model_capability"
							...
							>
								...
								>
									{ __( 'Add Model', 'bearded-media' ) }
								</button>
							</div>
						</SectionCard>

						<SectionCard
							title={ __( 'Registered Models', 'bearded-media' ) }
							icon={ <BoltIcon /> }
						>
							<div className="pipelines-stack">
								{ settings.manual_models.length === 0 ? (
									<p
										style={ {
											color: '#666',
											fontStyle: 'italic',
									...
									>
										{ __(
											'No models registered yet. Add one above to get started.',
											'bearded-media'
										) }
									</p>
								) : (
									settings.manual_models.map(
										( model, index ) => {
											let icon = null;
											if (
				...
				{ activeTab === 'credentials' && (
					...
					>
						<div className="sidebar-content">
							...
							<InputField
								id="openrouter_key"
								label={ __( 'OpenRouter', 'bearded-media' ) }
								type="password"
								placeholder="••••••••••••"
								value={ apiKeys.openrouter_key }
								onChange={ ( v ) =>
									updateKey( 'openrouter_key', v )
								}
							/>
						</div>
					</SectionCard>
