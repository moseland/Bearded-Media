 ...
 * @param {File}   root0.image     The image file to convert.
 * @param {number} root0.quality   The quality of the output image (0-100).
 * @param {number} root0.maxWidth  Maximum width constraint.
 * @param {number} root0.maxHeight Maximum height constraint.
 * @return {Promise<{webpBlob: Blob, fileName: string}>} A promise resolving to the webp blob and filename.
 */
export default async function webpfy( {
	...
	try {
		...
		await new Promise( ( resolve, reject ) => {
			...
			img.onerror = () =>
				reject(
					new Error( 'Failed to load image for WebP conversion.' )
				);
		} );
			...
			canvas.toBlob(
				( blob ) => {
					if ( blob ) {
						...
						reject(
							new Error( 'Failed to convert to WebP format.' )
						);
					}
				},
				'image/webp',
